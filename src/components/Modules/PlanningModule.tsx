...
function PlanCard({ plan, onDelete }: { plan: Plan; onDelete: () => void }) {
  ...
  const addMilestone = () => {
    ...
    updatePlan(plan.id, { milestones: [...plan.milestones, { id: nanoid(), title: newMs.trim(), done: false }] })
    setNewMs('')
  }

  const toggleMilestone = (msId: string) =>
    updatePlan(plan.id, { milestones: plan.milestones.map((m) => m.id === msId ? { ...m, done: !m.done } : m) })
    ...
    <div className="rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
      <div className="flex items-start gap-2">
        <div className="flex-1">
          ...
          {issue && <p className="text-xs text-orange-600 mt-0.5">→ {issue.title}</p>}
          {total > 0 && (
            <div className="mt-2">
        ...
        <div className="mt-3 space-y-2">
          <div className="space-y-1">
            {plan.milestones.map((ms) => (
              <div key={ms.id} className="flex items-center gap-2 group">
                ...
                <span className={`text-xs flex-1 ${ms.done ? 'line-through text-gray-400' : 'text-gray-700'}`}>
                  {ms.title}
                </span>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={addMilestone} disabled={!newMs.trim()}
              className="p-1.5 bg-gray-200 hover:bg-gray-300 disabled:opacity-40 rounded-lg transition-colors">
...
export default function PlanningModule() {
  ...
  const handleAdd = () => {
    ...
    const plan: Plan = {
      id: nanoid(), title: title.trim(),
      issueId: issueId || undefined,
      selfItemIds: selfIds, milestones: [],
      createdAt: new Date().toISOString(),
    }
    addPlan(plan)
    addMapNode({
      id: `node-plan-${plan.id}`, type: 'plan', label: plan.title, refId: plan.id,
      position: { x: 250 + Math.random() * 200, y: 350 + Math.random() * 100 },
    })
    setTitle(''); setIssueId(''); setSelfIds([])
  }
    ...
    <div className="flex flex-col h-full bg-white">
      <div className="p-4 border-b border-gray-200">
        ...
        <div className="space-y-2">
          ...
          {selfItems.length > 0 && (
            <div>
              <p className="text-xs text-gray-400 mb-1">活かす自分の要素（複数選択可）</p>
              <div className="flex flex-wrap gap-1">
                {selfItems.map((s) => (
                  <button key={s.id} onClick={() => toggleSelfId(s.id)}
                    className={`text-xs px-2 py-0.5 rounded-full border transition-colors ${
                      selfIds.includes(s.id) ? 'text-indigo-600 border-indigo-400 bg-indigo-50' : 'text-gray-400 border-gray-200 hover:text-gray-600'
                    }`}>
                    {s.content}
                  </button>
                ))}
